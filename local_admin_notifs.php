<!DOCTYPE html>
<?php include("local_admin_sidebar.php");
...
<html>
    ...
    <body>
        <h3 id='a'>Notifications Sent</h3>
        <table>
        <tr>
        <th>S.No</th>
            <th class="cid">Message Id</th>
            ...
            <th class="rvw">Notification</th>
            <th style="min-width: 200px;">Complaint</th>
            <th class="ra">Given At</th>
        </tr>
        <?php 
        include('config.php');
        $sql4 = "SELECT msg_id, given_at, msg, dept_name, complaint, complaints.complaint_id, user_id, complaints.authority_id as 'authority_id' from complaints join messages on complaints.complaint_id = messages.complaint_id join departments on departments.dept_id = complaints.dept_id join authority_details ad on ad.authority_id = complaints.authority_id where local_admin_id = '$user_id'";
        ...
        if($result4->num_rows > 0)
        {
            ...
            while($row = $result4->fetch_assoc())
            {
                ...
                $review = $row['msg'];
                $review_id = $row['msg_id'];
                
                $complaint = $row['complaint'];
                echo "<tr><td>".$i++."</td><td class='cid'>".$row["msg_id"]."</td><td class='ra'>".$row['complaint_id']."</td><td class='ra'>".$row['dept_name']."</td><td class='ra'>".$user_name."</td><td class='ra'>".$authority_name."</td><td class='rvw'>"."<button class='btn btn-success' onclick=\"view_review('$review', $comp_id, $review_id)\"><small>View Message</small></button>"."</td><td>"."<button class='btn btn-success' onclick=\"view_complaint('$complaint', $comp_id)\"><small>View Complaint</small></button>"."</td><td>".$row['given_at']."</td></tr>";
            }
        }
